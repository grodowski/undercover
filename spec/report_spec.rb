...
describe Undercover::Report do
  ...
  it 'builds a report with coverage metrics' do
    ...
    expect(module_results.map(&:name))
      .to eq(%w[BaconModule bar baz branch_missed branch_hit])
    ...
    expect(unflagged.map(&:name)).to eq(%w[BaconModule baz branch_hit])
    expect(unflagged[0].name).to eq('BaconModule')
    expect(unflagged[0].coverage_f).to eq(0.8)
