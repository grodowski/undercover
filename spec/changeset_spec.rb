...
describe Undercover::Changeset do
  shared_examples 'a changeset adapter' do |adapter_class|
    def build_changeset(dir, compare_base = nil, filter_set = nil)
      Undercover::Changeset.new(
        dir, compare_base, filter_set,
        adapter: described_adapter.new(dir, compare_base)
      )
    end

    let(:described_adapter) { adapter_class }

    it 'diffs index and staging area against HEAD' do
      changeset = build_changeset('spec/fixtures/test.git')
    ...
    it 'has all the changes agains base with compare_base arg' do
      changeset = build_changeset('spec/fixtures/test.git', 'master')
    ...
    it 'has a default last_modified with no changes' do
      changeset = build_changeset('spec/fixtures/empty.git', 'master')
    ...
    describe 'filtering' do
      it 'filters files using FilterSet in each_changed_line' do
        ...
        changeset = build_changeset('spec/fixtures/test.git', 'master', filter_set)
  ...
  context 'with RuggedAdapter' do
    include_examples 'a changeset adapter', Undercover::Changeset::RuggedAdapter
  end

  context 'with GitAdapter' do
    include_examples 'a changeset adapter', Undercover::Changeset::GitAdapter
  end
end
